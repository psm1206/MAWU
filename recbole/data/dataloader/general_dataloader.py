...
class FullSortEvalDataLoader(AbstractDataLoader):
    ...
    def _init_batch_size_and_step(self):
        batch_size = self.config['eval_batch_size']
        if not self.is_sequential:
            # interaction이 단위이기 때문에, 하나의 user에 대해 평가하면 item_num만큼의 interaction이 나옴
            # batch_num은 한 번에 평가할 user의 수 (설정한 batch_size에 따라 결정됨)
    ...
    def _next_batch_data(self):
        if not self.is_sequential:
            ...
            # positive_u: user index
            # positive_i: item index
