...
class Trainer(AbstractTrainer):
    ...
    def __init__(self, config, model):
        ...
        self.checkpoint_dir = os.path.join(config['checkpoint_dir'], f"{self.config['model']}/{self.config['dataset']}")
        ensure_dir(self.checkpoint_dir)
        self.local_time_name = '{}-{}-{}'.format(self.config['model'], self.config['dataset'], get_local_time())
        self.saved_model_file = os.path.join(self.checkpoint_dir, self.local_time_name + '.pth')
    ...
    def _train_epoch(self, train_data, epoch_idx, loss_func=None, show_progress=False, test_data=None):
    ...
    def fit(self, train_data, valid_data=None, verbose=True, saved=True, show_progress=False, callback_fn=None, test_data=None):
        ...
        # copy model .py file to checkpoint dir
        import shutil
        shutil.copyfile(os.path.join("recbole/model/general_recommender", self.config['model'].lower() + '.py'),
                        os.path.join(self.checkpoint_dir, self.local_time_name + '.py'))
        ...
        self.logger.info(set_color("Total (train & valid) time", 'blue') + ": %.2fs" % (train_time))
        self._add_hparam_to_tensorboard(self.best_valid_score)
        return self.best_valid_score, self.best_valid_result, round(train_time, 2)
    ...
    def evaluate(self, eval_data, load_best_model=True, model_file=None, show_progress=False):
        ...
        # default mode가 일단 full_sort_eval_dataloader 사용하게 되어 있음.
        ...
        for batch_idx, batched_data in enumerate(iter_data):
            # eval_func에서 model inference 시행
