...
class Config(object):
    ...
    def _set_train_neg_sample_args(self):
        ...
        if neg_sampling is None or neg_sampling == 'None':
            self.final_config_dict['train_neg_sample_args'] = {'strategy': 'none', 'in_batch': False}
        else:
            ...
            in_batch = False
            if 'in_batch' in neg_sampling.keys():
                in_batch = neg_sampling['in_batch']
                if in_batch:
                    sample_num = self.final_config_dict['train_batch_size'] - 1
